...
//Constructor
JoeCache::JoeCache(int base_assoc, int block_size, int cache_size, int address_width, bool write_policy, int passiveness) {
...
void JoeCache::write_cache(int address, int order) {
  ...
  JoeDirectorySet d = directory.at(index);
...
void JoeCache::read_cache(int address, int order) {
  int offset = address & ((int)pow(2, this->offset_length) - 1);
  int index = (address & (((int)pow(2, this->index_length) - 1)) << this->offset_length) >> this->offset_length;
  int tag = (address & (((int)pow(2, this->tag_length) - 1)) << (this->offset_length+this->index_length)) >> (this->offset_length+this->index_length);
...
void JoeCache::emplace_entry(int index, int tag, int order) {
  int oldest_index = 0;
...
void JoeCache::rebalance() {
  this->sorted_directory = directory;
  sort(sorted_directory.begin(), sorted_directory.end(), [](JoeDirectorySet a, JoeDirectorySet b) {return a.access_count>b.access_count;});
  DirectoryEntry ent;
  ent.tag = -1;
  ent.last_used = 0;

  for(int i = 0; i <=num_sets/8 - 1; i++) {
    directory.at(sorted_directory.at(i).index).assoc = base_assoc*4;
    directory.at(sorted_directory.at(i).index).set_tags.resize(base_assoc*4, ent);
  }

  for(int i = num_sets/8; i <=2*num_sets/8 - 1; i++) {
  ...
  for(int i = 2*num_sets/8; i < num_sets; i++) {
...
bool JoeCache::sort_helper(JoeDirectorySet i, JoeDirectorySet j) {
